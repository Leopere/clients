...
//!
//! This is the principled backbone for identifying a window. Rather than guessing whether a
//! window's AUMID "looks like" a PWA by its string shape, we resolve the AUMID against this
//! registry to obtain the app's true identity and real display name. Enumerated once per run;
//! failure is non-fatal (callers fall back to exe/product-name identity).
//!
//! Documented API path: `SHGetKnownFolderItem(FOLDERID_AppsFolder)` → `IShellItem`
//! `BindToHandler(BHID_EnumItems)` → `IEnumShellItems`; per item read `PKEY_AppUserModel_ID`
//! (`IShellItem2::GetString`) and the display name (`IShellItem::GetDisplayName`).

use std::ffi::c_void;

use windows::Win32::System::Com::CoTaskMemFree;
use windows::Win32::UI::Shell::{
    IEnumShellItems, IShellItem, IShellItem2, KF_FLAG_DEFAULT, SHGetKnownFolderItem,
    SIGDN_NORMALDISPLAY,
};
...
/// Load the AppsFolder registry. Never panics; on any COM failure returns an empty map and
/// callers simply fall back to exe/product-name identity.
pub(super) fn load() -> AppRegistry {
    let mut map = AppRegistry::new();
    // COM is already initialized by the caller (get_running_apps).
...
fn load_into(map: &mut AppRegistry) -> windows::core::Result<()> {
    ...
    unsafe {
        let folder: IShellItem = SHGetKnownFolderItem(&FOLDERID_APPS_FOLDER, KF_FLAG_DEFAULT, None)?;
...
/// Read an item's AUMID (the identity key) and display name. Skips items without an AUMID.
fn read_entry(item: &IShellItem) -> Option<(String, String)> {
