...
//! the machine and prints the results as a table. It makes **no assertions** — it's a developer
//! tool to eyeball what the enumeration returns, so it must be run on a Windows machine that
//! already has a normal set of apps open. It is not run in CI.
//!
//! Run it (from `desktop_native/`) with output shown:
//!
//! ```text
//! cargo test -p autotype --test integration_running_apps -- --nocapture
//! ```
...
fn print_running_apps() {
    ...
    let name_w = apps.iter().map(|a| a.display_name.len()).max().unwrap_or(4).max(4);

    println!("  {:<name_w$}  {}", "NAME", "PATH");
    println!("  {:<name_w$}  {}", "-".repeat(name_w), "----");
    for a in &apps {
        println!("  {:<name_w$}  {}", a.display_name, path_or_unknown(&a.path));
    }
    println!();
}
